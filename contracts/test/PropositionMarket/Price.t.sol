...
contract PriceTest is Test {
    ...
    function simulateExecutionPrice(uint256 supply, uint256 supplyOther, int256 deltaSupply) internal pure returns (uint256) {
    ...
    function runExecutionPriceTestCases(ExecutionPriceTestCase[] storage cases) internal view {
        for (uint256 i = 0; i < cases.length; i++) {
            if (cases[i].expectedRevert) {
                // 使用try-catch确认是否会revert
                try this.callExecutionPrice(cases[i].supply, cases[i].supplyOther, cases[i].deltaSupply) returns (uint256) {
                    // 如果没有revert，测试失败
                    console2.log("Test Case #", i, "should revert but did not");
                ...
                // 计算模拟的执行价格
                uint256 simulatedPrice = simulateExecutionPrice(
                    cases[i].supply, 
                    cases[i].supplyOther, 
                    cases[i].deltaSupply
                );
                ...
                if (!diffAcceptable) {
                    ...
                    console2.log("  supplyOther:", cases[i].supplyOther);
                    console2.log("  deltaSupply:", cases[i].deltaSupply);
                    console2.log("  executionPrice:", executionPrice);
                    console2.log("  simulatedPrice:", simulatedPrice);
                    console2.log("  priceDiff (basis points):", priceDiff);
                    console2.log("-----------------------");
                }
    ...
    // 用于try-catch测试
    function callExecutionPrice(uint256 supply, uint256 supplyOther, int256 deltaSupply) external pure returns (uint256) {
        return Price.getExecutionPrice(supply, supplyOther, deltaSupply);
    }

    function test_GetSpotPrice() public {
        delete testCases; // 清空数组
        testCases.push(TestCase(0, 0, 100000000000000000));
        testCases.push(TestCase(1 ether, 0, 105999000999000999));
        testCases.push(TestCase(0 ether, 1 ether, 100000000000000000));
        testCases.push(TestCase(1 ether, 1 ether, 105998003992015968));
    ...
    function test_GetExecutionPrice() public {
        ...
        // 初始情况
        executionPriceTestCases.push(ExecutionPriceTestCase(0, 0, 0 ether, false));
        executionPriceTestCases.push(ExecutionPriceTestCase(0, 0, -1 ether, true));
        executionPriceTestCases.push(ExecutionPriceTestCase(0, 0, 1 ether, false));
        executionPriceTestCases.push(ExecutionPriceTestCase(0, 0, 0.01 ether, false));
        executionPriceTestCases.push(ExecutionPriceTestCase(0, 0, 10000 ether, false));
        executionPriceTestCases.push(ExecutionPriceTestCase(0, 0, 10000000 ether, false));

        // 一方supply为0的情况
        executionPriceTestCases.push(ExecutionPriceTestCase(1 ether, 0, 1 ether, false));
        executionPriceTestCases.push(ExecutionPriceTestCase(1 ether, 0, -1 ether, false));
        executionPriceTestCases.push(ExecutionPriceTestCase(1000 ether, 0, 100 ether, false));
        executionPriceTestCases.push(ExecutionPriceTestCase(0, 1 ether, 1 ether, false));
        executionPriceTestCases.push(ExecutionPriceTestCase(0, 1 ether, -1 ether, true));
        executionPriceTestCases.push(ExecutionPriceTestCase(0, 1000 ether, 100 ether, false));

        // 买方
        executionPriceTestCases.push(ExecutionPriceTestCase(1 ether, 1 ether, 1 ether, false));
        executionPriceTestCases.push(ExecutionPriceTestCase(10 ether, 10 ether, 1 ether, false));
        executionPriceTestCases.push(ExecutionPriceTestCase(10 ether, 10 ether, 100 ether, false));
        executionPriceTestCases.push(ExecutionPriceTestCase(10000000 ether, 100000 ether, 10000 ether, false));
        executionPriceTestCases.push(ExecutionPriceTestCase(100000 ether, 10000000 ether, 10000 ether, false));

        // 卖方
        executionPriceTestCases.push(ExecutionPriceTestCase(1 ether, 1 ether, -1 ether, false));
        executionPriceTestCases.push(ExecutionPriceTestCase(10 ether, 10 ether, -1 ether, false));
        executionPriceTestCases.push(ExecutionPriceTestCase(10 ether, 10 ether, -100 ether, true));
        executionPriceTestCases.push(ExecutionPriceTestCase(10000000 ether, 100000 ether, -10000 ether, false));
        executionPriceTestCases.push(ExecutionPriceTestCase(100000 ether, 10000000 ether, -10000 ether, false));
        ...
        for (uint256 i = 0; i < 100; i++) {
            ...
            executionPriceTestCases.push(ExecutionPriceTestCase(supply, supplyOther, deltaSupply, false));
        }
